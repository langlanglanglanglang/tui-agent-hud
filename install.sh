#!/usr/bin/env bash
# cc-window-hud 安装器
#   1) tmux 底栏 HUD：status-right 调 cc-hud-title + status-interval 5
#   2) shell wrapper：claude / codex 启动时后台挂一个只盯本窗的 cc-watch（兼容 cc+codex）
...
# 删掉文件里旧的 cc-window-hud 块（含边界行）——awk 实现，跨 GNU/BSD 可移植
strip_block() {
...
cat >> "$TMUXCONF" <<EOF
# $MARK
# 底栏显示当前窗「红绿灯 + 编号 + 任务标题」；红绿灯由每窗 cc-watch 自监听维护
...
# 起 claude / codex 时后台挂一个只盯本窗的 cc-watch（幂等、窗关自退、只改自己窗名）
claude() { [ -n "\${TMUX:-}" ] && ( "$BIN/cc-watch" >/dev/null 2>&1 & ); command claude "\$@"; }
codex()  { [ -n "\${TMUX:-}" ] && ( "$BIN/cc-watch" >/dev/null 2>&1 & ); command codex "\$@"; }
# $MARKEND
EOF
...
echo "→ 已开的 cc/codex 窗要挂上监听：在该窗跑一次  $BIN/cc-watch &"
